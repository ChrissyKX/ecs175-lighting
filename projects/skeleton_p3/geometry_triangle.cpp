...
void
TriangleArrayObjects::BindWholeBuffer() {
  for (auto& m : meshes) {
...
TriangleArrayObjects::Render()
{
  ...
  for (auto& m : meshes) {
    ...
    if (!enable_painter_algorithm) {
      ...
      for (auto p : m.painter_tri_order) {
        // 1rst attribute buffer : vertices
        glEnableVertexAttribArray(0);
        glBindBuffer(GL_ARRAY_BUFFER, m.vbo_vertex);
        glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 9, &(m.vertices.get()[p.first * 9]), GL_STATIC_DRAW);
        glVertexAttribPointer(0, // attribute
                              3, // size
                              GL_FLOAT, // type
                              GL_FALSE, // normalized?
        ...
        glEnableVertexAttribArray(1);
        glBindBuffer(GL_ARRAY_BUFFER, m.vbo_texcoord);
        glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 6, &(m.texcoords.get()[p.first * 6]), GL_STATIC_DRAW);
        glVertexAttribPointer(1, // attribute
                              2, // size
                              GL_FLOAT, // type
                              GL_FALSE, // normalized?
        ...
        glEnableVertexAttribArray(2);
        glBindBuffer(GL_ARRAY_BUFFER, m.vbo_normal);
        glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 9, &(m.normals.get()[p.first * 9]), GL_STATIC_DRAW);
        glVertexAttribPointer(2, // attribute
                              3, // size
                              GL_FLOAT, // type
                              GL_FALSE, // normalized?
